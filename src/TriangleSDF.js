...
export default class TriangleSDF {
	...
	constructor( object ) {
		...
		this.triangles = this.extractTrianglesFromMeshes( object );
		this.triangleTexture = this.createTriangleTexture( this.triangles );
		this.normalTexture = this.createNormalTexture( this.triangles );
	...
	extractTrianglesFromMeshes( object ) {
		...
		object.traverse( obj => {
			...
			if ( obj.isMesh ) {
				...
				const geometry = obj.geometry.rotateX( - Math.PI / 2 ).rotateY( - Math.PI / 2 ).translate( 0, - 1, - 5 );
				...
				for ( let i = 0; i < count; i += 3 ) {
					...
					const triangle = { posA, posB, posC, normalA, normalB, normalC, material: obj.material };
	...
	createTriangleTexture( triangles ) {

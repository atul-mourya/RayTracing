 ...
 * - Reads ASVGF textures from context instead of direct pass reference
 * - Reads PathTracer G-buffer textures from context
 * - Listens to 'tile:changed' event for tile rendering
 * - Publishes sampling texture to context for PathTracer to use
 *
 * Events listened to:
 * - tile:changed - Updates tile bounds and mode
 * - pipeline:resize - Updates resolution
 *
 * Textures published to context:
 * - adaptiveSampling:output - Sampling guidance texture
 * - adaptiveSampling:heatmap - Debug visualization (if enabled)
 */
export class AdaptiveSamplingStage extends PipelineStage {
	...
	constructor( options = {} ) {
		...
		this.adaptiveSamplingMin = options.adaptiveSamplingMin || DEFAULT_STATE.adaptiveSamplingMin;
		this.adaptiveSamplingMax = options.adaptiveSamplingMax || DEFAULT_STATE.adaptiveSamplingMax;
		this.adaptiveSamplingVarianceThreshold = options.adaptiveSamplingVarianceThreshold || DEFAULT_STATE.adaptiveSamplingVarianceThreshold;
		this.showAdaptiveSamplingHelper = options.showAdaptiveSamplingHelper !== undefined ? options.showAdaptiveSamplingHelper : DEFAULT_STATE.showAdaptiveSamplingHelper;

		// REMOVED: Direct ASVGF pass reference (replaced with context access)
		// this.asvgfPass = null;
	...
	setupEventListeners() {
		...
		this.on( 'tile:changed', ( data ) => {
			...
			if ( data && data.tileBounds ) {
				...
				this.setTileMode( true, data.tileBounds );

			} else {

				this.setTileMode( false );
	...
	// REMOVED: setASVGFPass() - No longer needed, reads from context
	...
	render( context, writeBuffer ) {
		...
		// Get ASVGF temporal data from context instead of direct pass reference
		let hasASVGFColor = false;
		let hasASVGFVariance = false;

		const asvgfColorTexture = context.getTexture( 'asvgf:temporalColor' );
		const asvgfVarianceTexture = context.getTexture( 'asvgf:variance' );

		if ( asvgfColorTexture ) {

			this.material.uniforms.asvgfColorTexture.value = asvgfColorTexture;
			hasASVGFColor = true;

		}

		if ( asvgfVarianceTexture ) {

			this.material.uniforms.asvgfVarianceTexture.value = asvgfVarianceTexture;
			hasASVGFVariance = true;

		}

		// Get G-buffer textures from context (published by PathTracerStage)
		const normalDepthTexture = context.getTexture( 'pathtracer:normalDepth' );
		const currentColorTexture = context.getTexture( 'pathtracer:color' );
		...
		// Publish textures to context for other stages to use
		this.publishTexturesToContext( context );

	}

	/**
	 * Publish adaptive sampling textures to pipeline context
	 * @param {PipelineContext} context - Pipeline context
	 */
	publishTexturesToContext( context ) {

		// Publish sampling output texture
		context.setTexture( 'adaptiveSampling:output', this.renderTarget.texture );

		// Publish state values
		context.setState( 'adaptiveSamplingMax', this.adaptiveSamplingMax );
		context.setState( 'adaptiveSamplingMin', this.adaptiveSamplingMin );

		// Publish heatmap if enabled
		if ( this.showAdaptiveSamplingHelper && this.heatmapTarget ) {

			context.setTexture( 'adaptiveSampling:heatmap', this.heatmapTarget.texture );

		}

	}

	// DEPRECATED: setTextures() - No longer needed, reads from context
	// Keeping for backward compatibility but should not be called
	setTextures( currentColorTexture, normalDepthTexture ) {

		this.warn( 'setTextures() is deprecated - AdaptiveSamplingStage reads from context automatically' );
